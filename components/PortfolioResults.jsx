...
import {
    ...
    Info, AlertTriangle, CheckCircle, Layers, PieChart as PieIcon, HelpCircle
} from 'lucide-react';
...
const formatPercent = (value) => {
    ...
    return `${(value * 100).toFixed(2)}%`; // Assumes value is decimal (e.g. 0.05 for 5%)
};

// Helper for values that are already percentages or need specific formatting
const formatPercentDirect = (value) => {
    if (value === undefined || value === null) return '-';
    return `${value.toFixed(2)}%`;
};

const formatNumber = (value) => value ? value.toFixed(4) : '-';
...
export default function PortfolioResults({ data, config = { startYear: '2018', endYear: '2023' } }) {
    ...
    const assetColumns = [
        ...
        { key: 'annualized_return', label: 'Ann. Return', numeric: true, render: (val) => formatPercent(val) },
        { key: 'annualized_volatility', label: 'Volatility', numeric: true, render: (val) => formatPercent(val) },
        { key: 'max_drawdown', label: 'Max Drawdown', numeric: true, render: (val) => <span className="text-rose-400">{formatPercent(val)}</span> },
    ];

    // Prepare Drawdowns for SortableTable
    const drawdownColumns = [
        { key: 'depth', label: 'Depth', numeric: true, render: (val) => <span className="text-rose-400 font-bold">{formatPercentDirect(val * 100)}</span> },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="p-6 rounded-2xl bg-slate-800/40 border border-white/5 shadow-xl backdrop-blur-sm relative overflow-hidden group hover:bg-slate-800/50 transition-all">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="text-3xl font-bold text-blue-400 tracking-tight">{formatPercentDirect(data.metrics.expectedReturn)}</div>
                ...
                <div className="p-6 rounded-2xl bg-slate-800/40 border border-white/5 shadow-xl backdrop-blur-sm relative overflow-hidden group hover:bg-slate-800/50 transition-all">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="text-3xl font-bold text-rose-400 tracking-tight">{formatPercentDirect(data.metrics.maxDrawdown)}</div>
                        <div className="mt-4 h-10 w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    <div className="p-8 rounded-3xl bg-slate-800/40 border border-white/5 shadow-xl backdrop-blur-sm">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="flex justify-between py-3 border-b border-slate-700/50 hover:bg-white/5 px-2 rounded-lg transition-colors">
                                                ...
                                                <span className={`font-mono font-bold text-lg ${data.metrics.alpha > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{data.metrics.alpha ? formatPercentDirect(data.metrics.alpha * 100) : "N/A"}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
